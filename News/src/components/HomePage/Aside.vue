<template>
  <el-aside :width="$store.state.isCollapse ? '64px' : '200px'">
    <p class="siteTitle" @click="goHome">
      <img src="../../assets/logo.png" alt="" />
      <span v-show="!$store.state.isCollapse">企业门户管理</span>
    </p>
    <!-- 导航菜单 -->
    <el-menu :collapse="$store.state.isCollapse" :router="true" :default-active="route.fullPath" text-color="#fff">
      <!-- 首页 -->
      ...
      <!-- 用户管理 -->
      <el-sub-menu index="/user-manage" v-if="store.state.userInfo.role === 1">
        <template #title>
          <el-icon>
            <UserFilled />
          </el-icon>
